...
import com.dianping.shadow.advice.AdviceBeanFactory;
 ...
 */
public class ParsePointcutAnnotation {
    private static final Logger LOG = LoggerFactory.getLogger(ParsePointcutAnnotation.class);
    ...
    private static class MemberParseFilter implements AbstractScanner.ScannerFilter<Member> {
        ...
        public boolean accept(Member member) {
            ...
            for (Iterator<AspectDefinition> iterator = aspectResolver.iterator(); iterator.hasNext(); ) {
                ...
                if (Modifier.isAbstract(member.getModifiers()) || Modifier.isNative(member.getModifiers())) {
                    LOG.debug("Skip {}, current method is abstract or native", member.getName());
                    continue;
                }
                // 获取注解对象
                ...
                if (annotation != null) {
                    ...
                    AspectContext.getInstance().addJoinPoint(JoinPointBeanFactory.create(member)
                            , AdviceBeanFactory.create(aspectDefinition, PointcutBeanFactory.create(annotation)));
                }
            }
            // 无需缓存, 不需要收录Method
            return false;
